...
def commandprocessor(command,username):
    ...
    if command == "aisw":
        ...
        response='None'
        loop = asyncio.get_running_loop()
        loop.create_task(sendfault("**[Ошибка] [Команда]** Ошибка при исполнении команды, команда есть в списке но нет вывода"))
    return response
       ...
       pass
        ...
        try:
           response2=response.text.lower().strip()
            ...
            response2 = "None"
            loop = asyncio.get_running_loop()
            loop.create_task(sendfault(f"**[Ошибка] [Адаптер http]** Получены данные из которых нельзя получить текст json: {response.json} statuscode: {response.status_code} "))
        if "err" in response2:
            loop = asyncio.get_running_loop()
            loop.create_task(sendfault(
                f"**[Ошибка][Адаптер http]** Ошибка сервера {response2}"))
            response2 = None
        ...
        if len(splited) > 1:
            ...
            loop = asyncio.get_running_loop()
            loop.create_task(sendfault(
                   f"*[Дебаг] [http обучение]* Сообщение: {question} Ответ: {answer}  Разрешена отправка: {statelearn}"))
            response2=None
            if statelearn:
                requests.get(f'http://{httpfbhost}:{httpfbport}/get_answer', params={'text': question, 'reply_text': answer, 'space': 'matrix'})
        return(response2)
bot.run()
